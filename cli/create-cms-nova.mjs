...
// --- Files that should NEVER be auto-overwritten during upgrade ---
...
// --- Files that need special merge handling (warn user) ---
...
function createProject(projectName) {
    ...
    console.log(`   ${c.dim('npx prisma db push && npx prisma generate')}`);
...
async function upgradeProject(opts) {
    ...
    // 9) Filter out NEVER_OVERWRITE files
    const filesToProcess = changedFiles.filter(f => !NEVER_OVERWRITE.has(f));
...
async function cleanupDeletedFiles(interactive, baseRef, targetRef) {
    ...
    // Only files that still exist locally
    const zombieFiles = deletedInTemplate.filter(f => fs.existsSync(path.join(process.cwd(), f)));
