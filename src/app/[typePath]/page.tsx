    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="mx-auto max-w-5xl px-6 py-12">
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">
          {contentType ? contentType.name || typePath : typePath}
        </h1>
        ...
        {entries.length === 0 ? (
          <div className="mt-8 rounded-xl border border-gray-200 dark:border-gray-800 p-8 text-center bg-white/70 dark:bg-gray-900/70">
            <p className="text-sm text-gray-600 dark:text-gray-400">No published entries yet.</p>
          </div>
        ) : (
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {entries.map((e) => {
              ...
              if (!slug) return null
              const title: string =
                data.title || data.titulo || data.headline || data.heading || slug
