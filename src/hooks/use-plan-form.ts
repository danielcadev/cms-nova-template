    ...
    try {
      ...
      if (isCircuit) {
        ...
        if (circuitDestinations.length > 0) {
          ...
          if (fallbackMatch) {
            const keywords = fallbackMatch[1]
              .split(/\s+y\s+|\s*,\s*/)
    ...
    try {
      ...
      if (extractedText || mainDestination) {
        ...
        if (!currentTransfers || currentTransfers.trim() === '') {
          const transfersTitle = mainDestination
            ? `PLAN ${mainDestination.toUpperCase()}`
            : extractedText
              ? `PLAN ${extractedText.toUpperCase()}`
              : ''
        ...
        if (!currentHoliday || currentHoliday.trim() === '') {
          const holidayTitle = mainDestination
            ? `VACACIONES ${mainDestination.toUpperCase()}`
            : extractedText
              ? `VACACIONES ${extractedText.toUpperCase()}`
              : ''
        ...
        if (!currentAttractionsText || currentAttractionsText.trim() === '') {
          form.setValue(
            'attractionsText',
            'Tour guiado por los principales atractivos turísticos con entradas incluidas.',
            {
              shouldDirty: true,
              shouldValidate: false,
            },
        ...
        if (!currentTransfersText || currentTransfersText.trim() === '') {
          form.setValue(
            'transfersText',
            'Hotel en ubicación estratégica, traslados privados y tours guiados.',
            {
              shouldDirty: true,
              shouldValidate: false,
            },
        ...
        if (!currentHolidayText || currentHolidayText.trim() === '') {
          form.setValue(
            'holidayText',
            'Experiencia completa con los mejores atractivos turísticos de la región.',
            {
              shouldDirty: true,
              shouldValidate: false,
            },
