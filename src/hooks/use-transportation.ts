...
import type { PlanFormValues, TransportOption } from '@/schemas/plan'
...
export function useTransportation({
  ...
  const isGroundTransportAvailable =
    transportType !== 'air-only' && transportType !== 'air-temporary'
  ...
  const isAirTransportTemporary =
    transportType === 'air-temporary' ||
    form
      ?.getValues('transportOptions')
      ?.some(
        (option) =>
  ...
  const getTransportSummary = () => {
    ...
    return {
      ...
      message:
        'Transporte terrestre incluido en el plan. No es necesario que los pasajeros organicen su propio transporte.',
      cost: 0,
      isTemporary: false,
    }
  }
