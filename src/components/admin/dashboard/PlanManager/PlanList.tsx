...
import { CheckCircle, Edit2, ExternalLink, XCircle } from 'lucide-react'
          ...
          >
            ...
            >
              <div key={`left-content-${planKey}`} className="flex items-center gap-5">
                {/* Miniatura de imagen */}
                <div
                  key={`image-container-${planKey}`}
                  className="relative h-16 w-24 rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex-shrink-0"
                >
                  {plan.mainImage?.url ? (
                    <Image
                      key={`image-${planKey}`}
