...
export function HeroSection({ badge, title, description, features, stats }: HeroSectionProps) {
  return (
    <div className="text-center max-w-4xl mx-auto">
      {/* Badge */}
      <div
        className={`inline-flex items-center gap-3 ${badge.color} backdrop-blur-sm px-8 py-4 rounded-3xl border border-white/30 mb-8 shadow-lg`}
      >
      ...
      {/* Description */}
      <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed mb-8">{description}</p>

      {/* Features */}
      {features && (
        <div className="flex items-center justify-center gap-8 mb-8">
          {features.map((feature, index) => (
            <div key={`feature-${index}-${feature.label}`} className="text-center">
              <div
                className={`w-12 h-12 ${feature.color} rounded-2xl flex items-center justify-center mx-auto mb-2 shadow-lg`}
              >
