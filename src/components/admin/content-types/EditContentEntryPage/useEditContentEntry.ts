...
export function useEditContentEntry(entry: ContentEntry) {
  ...
  const titleField = entry.contentType.fields.find(
    (f) =>
      f.apiIdentifier === 'title' ||
      f.apiIdentifier === 'name' ||
      f.apiIdentifier === 'headline' ||
      f.apiIdentifier === 'titulo' ||
      f.apiIdentifier === 'nombre',
  )
  const imageField = entry.contentType.fields.find(
    (f) =>
      f.type === 'MEDIA' &&
      (f.apiIdentifier === 'mainImage' ||
        f.apiIdentifier === 'image' ||
        f.apiIdentifier === 'cover' ||
        f.apiIdentifier === 'featuredImage' ||
        f.apiIdentifier === 'imagen'),
  )
  ...
  const metaTitleField = entry.contentType.fields.find(
    (f) =>
      f.apiIdentifier === 'metaTitle' ||
      f.apiIdentifier === 'seoTitle' ||
      f.apiIdentifier === 'meta_title',
  )
  const metaDescriptionField = entry.contentType.fields.find(
    (f) =>
      f.apiIdentifier === 'metaDescription' ||
      f.apiIdentifier === 'seoDescription' ||
      f.apiIdentifier === 'meta_description',
  )
  const imageAltField = entry.contentType.fields.find(
    (f) =>
      f.apiIdentifier === 'imageAlt' ||
      f.apiIdentifier === 'altText' ||
      f.apiIdentifier === 'alt_text',
  )
  const tagsField = entry.contentType.fields.find(
    (f) => f.apiIdentifier === 'tags' || f.apiIdentifier === 'keywords',
  )
  ...
  const form = useForm({
    defaultValues: {
      ...
      ...(metaTitleField && entry.seoOptions
        ? { [metaTitleField.apiIdentifier]: (entry.seoOptions as any).metaTitle }
        : {}),
      ...(metaDescriptionField && entry.seoOptions
        ? { [metaDescriptionField.apiIdentifier]: (entry.seoOptions as any).metaDescription }
        : {}),
      ...(imageAltField && entry.seoOptions
        ? { [imageAltField.apiIdentifier]: (entry.seoOptions as any).imageAlt }
        : {}),
      ...(tagsField
        ? {
            [tagsField.apiIdentifier]: Array.isArray(entry.tags)
              ? entry.tags.join(', ')
              : entry.tags,
          }
        : {}),
      ...
      ...(() => {
        const catField = entry.contentType.fields.find(
          (f) => f.apiIdentifier === 'category' || f.apiIdentifier === 'categoria',
        )
  ...
  const [typePath, _setTypePath] = useState<string>(() =>
            ...
            .map((segment: string) => {
              if (segment.startsWith('[') && segment.endsWith(']')) {
                ...
                if (
                  paramKey === 'slug' ||
                  paramKey === '...slug' ||
                  paramKey === slugField?.apiIdentifier
                ) {
                  return ''
                }
