        ...
        <div className="max-w-lg mx-auto">
          ...
          {!searchTerm && (
            <Link href="/admin/dashboard/content-types/create">
              <button
                type="button"
                className="group relative px-8 py-4 bg-gradient-to-r from-[#0A84FF] to-[#64D2FF] text-white rounded-2xl font-semibold shadow-lg shadow-[#0A84FF]/20 hover:shadow-xl hover:shadow-[#0A84FF]/30 transition-all duration-300 hover:scale-105 hover:-translate-y-0.5"
              >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
      {contentTypes.map((contentType, index) => (
        <ContentTypeCard key={contentType.id} contentType={contentType} index={index} />
      ))}
    </div>
  )
}
