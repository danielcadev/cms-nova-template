    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid w-full grid-cols-4 bg-gray-200 rounded-lg p-1">
          <TabsTrigger
            value="fields"
            className="rounded-md text-sm py-2 px-3 data-[state=active]:bg-white data-[state=active]:text-gray-900 data-[state=active]:shadow-sm font-medium"
          >
            Campos
          </TabsTrigger>
          <TabsTrigger
              ...
              <DragOverlay>
                {activeField ? (
                  <div className="opacity-60">
                    <SortableFieldCard field={activeField} onDelete={() => {}} />
                  </div>
                ) : null}
              </DragOverlay>,
              document.body,
