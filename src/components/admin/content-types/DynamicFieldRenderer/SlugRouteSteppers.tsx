...
} from '@/modules/content/slug-actions'
...
export function SlugRouteSteppers({ wizardKey }: SlugRouteSteppersProps) {
  ...
  const handleDelete = async (type: 'region' | 'subRegion' | 'zone', id: string, name: string) => {
    if (
      confirm(`Are you sure you want to delete "${name}"? All dependent items will be removed.`)
    ) {
        ...
        {config.steps.map((step, index) => {
          ...
          return (
            <div key={step.param} className="space-y-1.5 relative">
              <div className="text-xs uppercase tracking-wider font-semibold text-zinc-500 dark:text-zinc-400 ml-1">
                {step.label}
              </div>
                        ...
                        <div className="p-2 space-y-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setModalCustomTitle(
                                step.modalLabel ? `Agregar ${step.modalLabel}` : undefined,
                              )
                      ...
                      <CommandGroup>
                        <CommandItem
                          onSelect={() => {
                            ...
                            setModalParentName(parentName)
                            setModalCustomTitle(
                              step.modalLabel ? `Add ${step.modalLabel}` : undefined,
                            )
                            setModalInitialValue(searchQuery)
                            setModalOpen(true)
                            setOpenStep(null)
