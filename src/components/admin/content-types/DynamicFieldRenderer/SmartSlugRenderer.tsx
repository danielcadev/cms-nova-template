...
import { SLUG_WIZARDS } from '@/modules/content/slug-config'
...
export function SmartSlugRenderer({
  ...
  const routeParams = slugRoute
    ? Array.from(slugRoute.matchAll(/\[([^\]]+)\]/g)).map((m) => m[1])
    : []
  ...
  let activeWizardKey: string | undefined
  ...
  const wizardParams = activeWizardKey
    ? SLUG_WIZARDS[activeWizardKey].steps.map((s) => s.param)
    : []
  ...
  // Watch dependent fields (without calling hooks inside a loop)
  const paramFieldValues = useWatch({
    control,
    name: paramFields.map((f) => f.apiIdentifier),
  })
  ...
  if (slugRoute) {
    ...
    const usesSelf =
      slugRoute.includes('[slug]') ||
      slugRoute.includes('[...slug]') ||
      slugRoute.includes(`[${field.apiIdentifier}]`)
    ...
    if (!usesSelf) {
      ...
      const selfPlaceholder = slugRoute.includes('[slug]')
        ? '[slug]'
        : slugRoute.includes('[...slug]')
          ? '[...slug]'
          : `[${field.apiIdentifier}]`
  ...
  return (
    <div className="space-y-4">
      ...
      {activeWizardKey && <SlugRouteSteppers wizardKey={activeWizardKey} />}
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="p-4 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            {paramFields.map((paramField, idx) => {
              const paramValue = Array.isArray(paramFieldValues) ? paramFieldValues[idx] : undefined
              const inputId = `smart-field-${paramField.id}`

              return (
                <div key={paramField.id} className="relative">
                  <label
                    htmlFor={inputId}
                    className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 mb-1.5 ml-1 block uppercase tracking-wider"
                  >
                    {t('step', { number: idx + 1 + (activeWizardKey ? 3 : 0) })}:{' '}
                    {paramField.label.replace('Lista de Selección', '').replace('Name', '')}
                  </label>
                  <DynamicFieldRenderer
                    field={paramField}
                    value={paramValue}
                    onChange={(val) =>
                      setValue(paramField.apiIdentifier, val, {
                        shouldDirty: true,
                        shouldValidate: true,
                      })
                    }
