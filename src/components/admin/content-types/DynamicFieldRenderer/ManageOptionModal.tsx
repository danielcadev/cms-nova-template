import { CheckCircle2, Edit3, FileText, List, Loader2, Plus } from 'lucide-react'
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <DialogTitle className="text-2xl font-bold tracking-tight">
                {customTitle || `Add ${typeLabels[type]}`}
              </DialogTitle>
              <DialogDescription className="text-zinc-400 text-sm mt-1">
                {parentName ? `Inside "${parentName}"` : 'New top-level location'}
              </DialogDescription>
            </div>
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <Label
                htmlFor={nameId}
                className="text-sm font-semibold text-zinc-600 flex items-center gap-2"
              >
                {isBulkMode ? (
            ...
            {isBulkMode ? (
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder={`Example: ${type === 'region' ? 'Caribbean' : 'Cartagena'}`}
                />
              </div>
            )}
            ...
            <p className="text-[11px] text-zinc-400 leading-relaxed px-1">
              {isBulkMode
                ? 'Paste or type one name per line. You can also use commas.'
          ...
          <div className="flex items-center gap-3 pt-4">
            ...
            >
              ...
              {isBulkMode ? `Add ${itemCount} items` : 'Save item'}
            </Button>
          </div>
        </div>
      </DialogContent>
