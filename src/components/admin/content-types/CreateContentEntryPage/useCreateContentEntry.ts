...
export function useCreateContentEntry(contentType: ContentType) {
  ...
  const metaDescriptionField = contentType.fields.find(
    (f) =>
      (f.type === 'TEXT' || f.type === 'RICH_TEXT') &&
      /(metaDescription|meta_description|seoDescription|seo_desc)/i.test(f.apiIdentifier),
  )
  ...
  const tagsField = contentType.fields.find(
    (f) =>
      (f.type === 'TEXT' || f.type === 'RICH_TEXT') &&
      /(tags|etiquetas|tag)/i.test(f.apiIdentifier),
  )
  ...
  }, [formData, titleField, slugField, mounted, legacySlug, handleFieldChange])
  ...
  const getPayload = useCallback(() => {
    ...
    return payload
  }, [formData, legacySlug, slugField, titleField, typePath])
