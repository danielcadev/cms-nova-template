import { Calendar, Database, Edit, Eye, FileText, PlusCircle, Sparkles } from 'lucide-react'
      ...
      >
        <HeroSection
          ...
          description={
            ...
            `Gestiona y organiza todas las entradas de ${contentType.name.toLowerCase()}`
          }
          stats={[
            {
        ...
        <div className="flex justify-end mt-8">
          <Link href={`/admin/dashboard/content/${contentType.apiIdentifier}/create`}>
            <ThemedButton className="rounded-2xl px-8 py-4 flex items-center gap-3 shadow-lg hover:shadow-xl transition-all backdrop-blur-sm bg-green-600 hover:bg-green-700 text-white">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 overflow-hidden">
          <div className="p-8 border-b border-gray-100">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-2xl font-bold text-gray-900">Todas las Entradas</h3>
                <p className="text-gray-600">
                  {contentType.entries.length}{' '}
                  {contentType.entries.length === 1 ? 'entrada' : 'entradas'} en total
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {contentType.entries.map((entry, index) => (
        <EntryCard key={entry.id} entry={entry} contentType={contentType} index={index} />
      ))}
    </div>
  )
}
...
function EntryCard({
  ...
  const title =
    entryData?.title || entryData?.name || entryData?.titulo || `Entrada #${entry.id.slice(-6)}`
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/admin/dashboard/content/${contentType.apiIdentifier}/edit/${entry.id}`}
            className="flex-1"
          >
            <ThemedButton className="w-full rounded-2xl border-gray-200 hover:bg-gray-50 text-gray-700 py-3">
          ...
          <ThemedButton
            variantTone="ghost"
            size="icon"
            className="text-green-600 hover:bg-green-50 rounded-2xl w-12 h-12"
          >
...
function EmptyState({ contentType }: { contentType: ContentType }) {
  return (
    <div className="text-center py-20">
      ...
      <h3 className="text-3xl font-bold text-gray-800 mb-6">No hay entradas todavía</h3>
      <p className="text-gray-600 mb-10 text-lg leading-relaxed max-w-md mx-auto">
        Crea tu primera entrada de {contentType.name.toLowerCase()} para comenzar a organizar tu
        contenido.
      </p>
      <Link href={`/admin/dashboard/content/${contentType.apiIdentifier}/create`}>
        <ThemedButton className="bg-green-600 hover:bg-green-700 text-white rounded-2xl px-8 py-4 text-lg font-semibold shadow-lg">
