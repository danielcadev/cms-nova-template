...
const URLPreview = memo(({ categoryAlias, destinationSlug, articleAlias }: { categoryAlias: string; destinationSlug: string; articleAlias: string;}) => (
    <div className="text-sm font-mono text-slate-800 break-all bg-slate-100 p-4 rounded-xl border border-slate-200">
        <span className="text-slate-400">tudominio.com</span>
        <span className="text-blue-600 font-medium">/</span><span className="text-blue-700">{categoryAlias||'categoria'}</span>
        <span className="text-blue-600 font-medium">/</span><span className="text-blue-700">{destinationSlug||'destino'}</span>
        <span className="text-blue-600 font-medium">/</span><span className="text-blue-700">{articleAlias||'mi-plan'}</span>
    </div>
));
        ...
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                <div className="md:col-span-1">
                    <h3 className="text-lg font-semibold text-slate-900">Imagen de Portada</h3>
                    <p className="text-sm text-slate-500 mt-1">Esta será la primera impresión de tu plan. Elige una imagen horizontal y de alta calidad.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                 <div className="md:col-span-1">
                    <h3 className="text-lg font-semibold text-slate-900">Detalles del Destino</h3>
                    <p className="text-sm text-slate-500 mt-1">Selecciona el destino principal de tu base de datos y especifica sus características.</p>
                </div>
                 <div className="md:col-span-2 space-y-6">
                    <FormField control={control} name="destinationId" render={({ field }) => (
                        <FormItem><FormLabel>Destino Principal</FormLabel><FormControl><OriginalCombobox value={field.value} onChange={field.onChange} options={destinations} onCreate={handleCreateDestination} placeholder={isLoadingDest ? "Cargando destinos..." : "Busca un destino o crea uno nuevo"} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <div className="flex items-center justify-between bg-slate-50 rounded-xl p-4 border"><div className="space-y-1"><FormLabel className="font-semibold text-slate-800">¿Es un plan terrestre?</FormLabel><p className="text-sm text-slate-500">Actívalo si el principal medio de transporte para llegar es por tierra.</p></div><FormField control={control} name="allowGroundTransport" render={({ field }) => <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl>} /></div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                <div className="md:col-span-1">
                    <h3 className="text-lg font-semibold text-slate-900">Título y Dirección Web</h3>
                    <p className="text-sm text-slate-500 mt-1">El nombre de tu plan y cómo se verá en la barra de direcciones del navegador. Clave para el SEO.</p>
                </div>
                <div className="md:col-span-2 space-y-6">
                    <FormField control={control} name="mainTitle" render={({ field }) => (
                        <FormItem><FormLabel>Título Principal del Plan</FormLabel><FormControl><Input {...field} placeholder="Ej: Circuito Mágico por la Riviera Maya" /></FormControl><FormMessage /></FormItem>
                    )} />
                    <div className="bg-slate-50 rounded-xl border p-4 space-y-4">
                        <p className="text-sm text-slate-600">Nuestras sugerencias inteligentes para la URL se basan en el título que escribas.</p>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <FormField control={control} name="categoryAlias" render={({ field }) => (
                                <FormItem><FormLabel>Categoría</FormLabel><FormControl><OriginalCombobox value={field.value} onChange={field.onChange} options={categoryOptions} onCreate={handleCreateCategory} placeholder="Elige..." /></FormControl><FormMessage /></FormItem>
                            )} />
                            <FormItem><FormLabel>Destino</FormLabel><FormControl><Input value={destinationSlug} readOnly placeholder="Automático" className="bg-slate-200" /></FormControl></FormItem>
                            <FormField control={control} name="articleAlias" render={({ field }) => (
                                <FormItem><FormLabel>Slug del Plan</FormLabel><FormControl><OriginalCombobox value={field.value} onChange={field.onChange} options={planSlugOptions} onCreate={handleCreatePlanSlug} placeholder="Elige o crea..." /></FormControl><FormMessage /></FormItem>
                            )} />
                        </div>
