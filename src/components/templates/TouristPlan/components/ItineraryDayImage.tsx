...
export function ItineraryDayImage({ fieldIndex }: ItineraryDayImageProps) {
  ...
  const itineraryFolder = useMemo(
    () =>
      getPlanMediaFolder({ slug: planSlug, fallbackTitle: planTitle, subfolder: ['itinerary'] }),
    [planSlug, planTitle],
  )
        ...
        {imageUrl ? (
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-3 w-full sm:w-auto">
              <label htmlFor={fileInputId} className="w-full sm:w-auto">
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
