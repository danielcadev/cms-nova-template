...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rl = rateLimit(request, { limit: 60, windowMs: 60_000, key: `plans:${id}:GET` })
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rl = rateLimit(request, { limit: 20, windowMs: 60_000, key: `plans:${id}:PATCH` })
    if (!rl.allowed) return R.error('Too many requests. Please try again later.', 429)

    const json = await request.json().catch(() => null)
    if (!json || typeof json !== 'object') {
      return R.validationError('Invalid data', [
        { field: 'body', message: 'Expected a JSON object', code: 'invalid_type' },
      ])
    }
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rl = rateLimit(request, { limit: 10, windowMs: 60_000, key: `plans:${id}:DELETE` })
    if (!rl.allowed) return R.error('Too many requests. Please try again later.', 429)
