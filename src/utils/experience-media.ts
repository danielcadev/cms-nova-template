...
interface ExperienceMediaFolderOptions {
  slug?: string | null
...
export function getExperienceMediaFolder({
  slug,
  fallbackTitle,
  subfolder,
}: ExperienceMediaFolderOptions): string {
  const normalizedSlug = normalizeSegment(slug) ?? normalizeSegment(fallbackTitle) ?? 'drafts'
  const segments: string[] = ['experiences', normalizedSlug]
